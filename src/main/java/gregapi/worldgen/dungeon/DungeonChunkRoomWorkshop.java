...
public class DungeonChunkRoomWorkshop extends DungeonChunkRoomEmpty {
	...
	public boolean generate(DungeonData aData) {
		...
		aData.set   ( 4, 1,  1, SIDE_UNKNOWN,  6011, UT.NBT.make(NBT_COLOR, DYES_INT[aData.mColor], NBT_PAINTED, T, NBT_FACING, SIDE_Z_POS, NBT_INV_LIST, UT.NBT.makeInv(NI, ST.make(aData.mPrimary  , 10000+aData.mRandom.nextInt(90001), 1))), T, T);
		aData.set   ( 4, 2,  1, SIDE_UNKNOWN,  6011, UT.NBT.make(NBT_COLOR, DYES_INT[aData.mColor], NBT_PAINTED, T, NBT_FACING, SIDE_Z_POS, NBT_INV_LIST, UT.NBT.makeInv(NI, ST.make(aData.mSecondary,  1000+aData.mRandom.nextInt( 9001), 1))), T, T);
		...
		if (tKeyIndex < aData.mGeneratedKeys.length) {
			...
			tInventory.appendTag(UT.NBT.makeShort(ST.save(aData.mKeyStacks[tKeyIndex]), "s", (short)(10+aData.mRandom.nextInt(18))));
		}
