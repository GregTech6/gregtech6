	...
	public static boolean generate(DungeonChunkData aData) {
		...
		if (aData.mRoomLayout[aData.mRoomX+1][aData.mRoomZ] == 0) {
			if (aData.mRandom.nextBoolean()) {
				...
				for (int i = 0; i < 2; i++) for (int j = 0; j < 3; j++) if (aData.mRandom.nextInt(3) > 0) for (int k = 0; k < 4; k++) {
		...
		if (aData.mRoomLayout[aData.mRoomX][aData.mRoomZ+1] == 0) {
			if (aData.mRandom.nextBoolean()) {
				int tType = aData.mRandom.nextInt(tIDs.length);
				for (int i = 0; i < 3; i++) for (int j = 0; j < 2; j++) if (aData.mRandom.nextInt(3) > 0) for (int k = 0; k < 4; k++) {
