 ...
 */
public class CoverLogisticsItemImport extends AbstractCoverAttachment {
		...
		if (aTool.equals(TOOL_magnifyingglass)) {
			if (aChatReturn != null) {
				if (aData.mNBTs[aCoverSide] == null) {
					aChatReturn.add("Imports Anything!");
					aData.mNBTs[aCoverSide] = null;
				} else {
					...
					if (ST.invalid(tStack)) {
						...
						aChatReturn.add("Imports: " + LH.Chat.CYAN + ST.regName(tStack) + LH.Chat.GRAY + " ; " + LH.Chat.CYAN + ST.meta_(tStack));
					}
				}
			}
			return 1;
