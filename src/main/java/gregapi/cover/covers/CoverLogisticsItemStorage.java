 ...
 */
public class CoverLogisticsItemStorage extends AbstractCoverAttachment {
			...
			if (aChatReturn != null) {
				if (aData.mNBTs[aCoverSide] == null) {
					...
					if (ST.invalid(tStack)) {
						...
						aChatReturn.add("Stores: " + LH.Chat.CYAN + ST.regName(tStack) + LH.Chat.GRAY + " ; " + LH.Chat.CYAN + ST.meta_(tStack));
					}
				}
			}
			return 1;
