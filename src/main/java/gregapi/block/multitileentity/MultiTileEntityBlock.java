...
public class MultiTileEntityBlock extends Block implements IBlockErrorable, IBlockOnWalkOver, IBlockSealable, IOxygenReliantBlock, IPaintableBlock, IBlockSyncDataAndCoversAndIDs, IRenderedBlock, ITileEntityProvider, IBlockToolable, IBlockRetrievable, IBlockMaterial, mcp.mobius.waila.api.IWailaBlock {
	...
	public final void breakBlock(World aWorld, int aX, int aY, int aZ, Block aBlock, int aMetaData) {
		...
		sTemporaryTileEntity.set(aTileEntity);
		...
		if (aTileEntity instanceof IMTE_HasMultiBlockMachineRelevantData && ((IMTE_HasMultiBlockMachineRelevantData)aTileEntity).hasMultiBlockMachineRelevantData()) ITileEntityMachineBlockUpdateable.Util.causeMachineUpdate(aWorld, new ChunkCoordinates(aX, aY, aZ), this, (byte)aMetaData, T);
	...
	@Override public final ArrayList<ItemStack> getDrops(World aWorld, int aX, int aY, int aZ, int aUnusableMetaData, int aFortune) {TileEntity aTileEntity = aWorld.getTileEntity(aX, aY, aZ); if (aTileEntity == null) aTileEntity = sTemporaryTileEntity.get(); if (aTileEntity == null || aTileEntity.xCoord != aX || aTileEntity.yCoord != aY || aTileEntity.zCoord != aZ) aTileEntity = null; if (aTileEntity instanceof IMTE_GetDrops) return ((IMTE_GetDrops)aTileEntity).getDrops(aFortune, F); return new ArrayListNoNulls<>();}
	...
	@Override public final void receiveBlockError(IBlockAccess aWorld, int aX, int aY, int aZ, String aError) {TileEntity tTileEntity = aWorld.getTileEntity(aX, aY, aZ); if (tTileEntity instanceof ITileEntity) {((ITileEntity)tTileEntity).setError(aError); ((World)aWorld).markBlockForUpdate(aX, aY, aZ);}}
	...
	@Override @Optional.Method(modid = ModIDs.WAILA) public List<String> getWailaHead(ItemStack aStack, List<String> aTooltips, IWailaDataAccessor aAccessor, IWailaConfigHandler aConfig) {return aTooltips;}
