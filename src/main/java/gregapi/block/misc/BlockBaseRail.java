...
public class BlockBaseRail extends BlockRailBase implements IBlockBase, IBlockSealable, IBlockToolable {
	...
	public float getRailMaxSpeed(World aWorld, EntityMinecart aCart, int aX, int aY, int aZ) {
		...
		case  0:
			if (WD.block(aWorld, aX  , aY, aZ+1) instanceof BlockRailBase && (WD.meta(aWorld, aX  , aY, aZ+1) & 7) == 0
			&&  WD.block(aWorld, aX  , aY, aZ-1) instanceof BlockRailBase && (WD.meta(aWorld, aX  , aY, aZ-1) & 7) == 0) return mSpeed;
		case  1:
			if (WD.block(aWorld, aX+1, aY, aZ  ) instanceof BlockRailBase && (WD.meta(aWorld, aX+1, aY, aZ  ) & 7) == 1
			&&  WD.block(aWorld, aX-1, aY, aZ  ) instanceof BlockRailBase && (WD.meta(aWorld, aX-1, aY, aZ  ) & 7) == 1) return mSpeed;
		default:
