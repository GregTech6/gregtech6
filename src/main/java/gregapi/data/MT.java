...
public class MT {
	...
	static OreDictMaterial emerald      (int aID, String aNameOreDict, TextureSet[] aSets, long aR, long aG, long aB, byte aColor)  {return valgemelec      (aID, aNameOreDict, aSets, aR, aG, aB, 127).lens(aColor).put(MELTING).aspects(TC.VITREUS, 3, TC.LUCRUM, 3).qual(3, 9.0, 128, 2).setSmelting(Be   ,   U36).addReRegistrations(ANY.Emerald);}
	static OreDictMaterial sapphire     (int aID, String aNameOreDict, TextureSet[] aSets, long aR, long aG, long aB, byte aColor)  {return valgemcent      (aID, aNameOreDict, aSets, aR, aG, aB, 127).lens(aColor).put(MELTING).aspects(TC.VITREUS, 3, TC.LUCRUM, 4).qual(3, 7.0, 512, 3).setSmelting(Al2O3, 3*U4 ).addReRegistrations(ANY.Sapphire);}
