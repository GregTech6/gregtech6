...
public class Loader_Recipes_Mariculture implements Runnable {
	@Override
	public void run() {
		if (MD.MaCu.mLoaded && MD.ENCHIRIDION.mLoaded) {OUT.println("GT_Mod: Doing Mariculture Recipes.");
			
			CR.remout(IL.MaCu_Polished_Logs.get(1));
			CR.remout(IL.MaCu_Polished_Planks.get(1));
