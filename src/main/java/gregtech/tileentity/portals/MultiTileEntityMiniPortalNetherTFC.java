 ...
 */
public class MultiTileEntityMiniPortalNetherTFC extends MultiTileEntityMiniPortal implements IMTE_RandomDisplayTick {
	public static List<MultiTileEntityMiniPortal>
	sListNetherSide = new ArrayListNoNulls<>(),
	sListTFCSide = new ArrayListNoNulls<>();
	
	@Override public List<MultiTileEntityMiniPortal> getPortalListA() {return sListTFCSide;}
	@Override public List<MultiTileEntityMiniPortal> getPortalListB() {return sListNetherSide;}
	
	static {
		LH.add("gt.tileentity.portal.nether.tfc.tooltip.1", "Only works between the Nether and the TFCWorld with a x8 Distance Factor!");
		LH.add("gt.tileentity.portal.nether.tfc.tooltip.2", "Margin of Error to still work: 128 Meters.");
	}
	
	@Override
	public void addToolTips2(List<String> aList, ItemStack aStack, boolean aF3_H) {
		aList.add(Chat.CYAN     + LH.get("gt.tileentity.portal.nether.tfc.tooltip.1"));
		aList.add(Chat.CYAN     + LH.get("gt.tileentity.portal.nether.tfc.tooltip.2"));
	...
	public void findTargetPortal() {
		...
		if (worldObj != null && isServerSide()) {
			if (worldObj.provider.dimensionId == DIM_TFC) {
				...
				long tShortestDistance = 128*128;
				for (MultiTileEntityMiniPortal tTarget : sListTFCSide) if (tTarget != this && !tTarget.isDead()) {
	...
	public void addThisPortalToLists() {
		if (worldObj != null && isServerSide()) {
			if (worldObj.provider.dimensionId == DIM_TFC) {
				if (!sListTFCSide.contains(this)) sListTFCSide.add(this);
				...
				for (MultiTileEntityMiniPortal tPortal : sListTFCSide) tPortal.findTargetPortal();
				findTargetPortal();
			} else {
				setPortalInactive();
			}
	...
	@Override public String getTileEntityName() {return "gt.multitileentity.portal.nether.tfc";}
}
