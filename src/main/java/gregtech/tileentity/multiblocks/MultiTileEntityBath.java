	...
	public boolean checkStructure2() {
		...
		if (worldObj.blockExists(tX-2, tY, tZ-2) && worldObj.blockExists(tX+2, tY, tZ-2) && worldObj.blockExists(tX-2, tY, tZ+2) && worldObj.blockExists(tX+2, tY, tZ+2)) {
            ...
            for (sY = 0; sY < 2; sY++) {
                for (sZ = 0; sZ < 5; sZ++) {
                    for (sX = 0; sX < 5; sX++) {
                        if (!ITileEntityMultiBlockController.Util.checkAndSetTarget(this, tX + sX, tY + sY, tZ + sZ, 18002, getMultiTileEntityRegistryID(), 0, MultiTileEntityMultiBlockPart.ONLY_ITEM_FLUID))
                            tSuccess = F;
                    }
                }
            }
