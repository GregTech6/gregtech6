...
public class MultiItemTechnological extends MultiItemRandom {
	...
	public void addItems() {
		...
		IL.Electrode_FR_Diamond				.set(addItem(tLastID = 29992, "Electrode (Diamond)"				, "Needs Glass Tube"								, new OreDictItemData(MT.Diamond		, 5*U4, MT.Redstone, U2), TC.stack(TC.ELECTRUM, 1), MD.FR.mLoaded ? null : TD.Creative.HIDDEN));
		...
		RM.Press.addRecipeX(T, 16, 64, new ItemStack[] {OP.stick.mat(MT.Diamond			, 2), OP.bolt.mat(MT.Diamond		, 2), OP.dustSmall.mat(MT.Redstone, 2)}, IL.Electrode_FR_Diamond	.get(1));
		...
		RM.Press.addRecipeX(T, 16, 64, new ItemStack[] {OP.stick.mat(MT.Diamond			, 4), OP.bolt.mat(MT.Diamond		, 4), OP.dust.mat(MT.Redstone, 1)}, IL.Electrode_FR_Diamond		.get(2));
		...
		IL.Circuit_Board_BC_Diamond.set(		addItem(tLastID = 30283, "Circuit Board (BC Diamond)"		, "Needs to be soldered properly"					, TC.stack(TC.FABRICO, 1), MD.BC_SILICON.mLoaded ? null : TD.Creative.HIDDEN, new OreDictItemData(OM.stack(MT.Signalum, U), OM.stack(ANY.SiO2,  U), OM.stack(MT.Plastic, U), OM.stack(MT.Redstone, 4*U), OM.stack(MT.Diamond, 4*U))));
		...
		IL.Circuit_BC_Diamond.set(				addItem(tLastID = 30383, "Circuit (BC Diamond)"				, "Made for tweaking Recipes"						, MD.BC_SILICON.mLoaded ? null : TD.Creative.HIDDEN, TC.stack(TC.COGNITO, 4), TC.stack(TC.LUCRUM, 2), TC.stack(TC.VITREUS, 2)		, new OreDictItemData(OM.stack(MT.Signalum, U), OM.stack(ANY.SiO2,  U), OM.stack(MT.Plastic, U), OM.stack(MT.Redstone, 4*U), OM.stack(MT.Diamond, 4*U))));
		...
		IL.Circuit_Crystal_Diamond.set(			addItem(tLastID = 30401, "Crystal Circuit (Diamond)"		, "Logic Diamond"									, TC.stack(TC.COGNITO, 3), TC.stack(TC.LUCRUM, 2), TC.stack(TC.VITREUS, 2), new OreDictItemData(OM.stack(MT.Diamond, U))));
		...
		IL.Processor_Crystal_Diamond.set(		addItem(tLastID = 30501, "Crystal Processor (Diamond)"		, "Logic Processor Circuit"							, TC.stack(TC.COGNITO, 5), TC.stack(TC.LUCRUM, 2), TC.stack(TC.VITREUS, 2), new OreDictItemData(MT.Pt, U, MT.Diamond, U)));
		...
		CR.shaped(IL.USB_HDD_1.get(1), CR.DEF_REV, "PLT", "dRW", "TCP", 'C', OD_CIRCUITS[3], 'W', IL.USB_Cable_1, 'L', IL.Comp_Laser_Gas_He, 'R', "record", 'P', OP.plate.dat(MT.Al				), 'T', OP.screw.dat(MT.Al				)); // TODO: Replace record with a CD (made of aluminium foils and plastic plates in a Press)
		CR.shaped(IL.USB_HDD_2.get(1), CR.DEF_REV, "PLT", "dRW", "TCP", 'C', OD_CIRCUITS[4], 'W', IL.USB_Cable_2, 'L', IL.Comp_Laser_Gas_He, 'R', "record", 'P', OP.plate.dat(MT.StainlessSteel	), 'T', OP.screw.dat(MT.StainlessSteel	));
		CR.shaped(IL.USB_HDD_3.get(1), CR.DEF_REV, "PLT", "dRW", "TCP", 'C', OD_CIRCUITS[5], 'W', IL.USB_Cable_3, 'L', IL.Comp_Laser_Gas_He, 'R', "record", 'P', OP.plate.dat(MT.Cr				), 'T', OP.screw.dat(MT.Cr				));
		CR.shaped(IL.USB_HDD_4.get(1), CR.DEF_REV, "PLT", "dRW", "TCP", 'C', OD_CIRCUITS[6], 'W', IL.USB_Cable_4, 'L', IL.Comp_Laser_Gas_He, 'R', "record", 'P', OP.plate.dat(MT.Ti				), 'T', OP.screw.dat(MT.Ti				));
