...
// TODO	RA.addAssemblerRecipe(OP.dust.mat(MT.Redstone, 1), OP.cell.mat(MT.Empty, 1), IL.Spray_Empty.get(1), 800, 1);
		...
		for (int i = 0; i < 8; i++) {
			...
			, addItem(++tLastID, "Wooden Bucket", "Water"									, TC.stack(TC.ARBOR, 2), TC.stack(TC.AQUA		, 2), new Behavior_Bucket_Simple(ST.make(Items.water_bucket, 1, 0))									, tData.copy(), FL.Water.make(1000), FL.DistW.make(1000)							, "container1000water")
			, addItem(++tLastID, "Wooden Bucket", "Milk (you cannot drink out of Buckets!)"	, TC.stack(TC.ARBOR, 2), TC.stack(TC.SANO		, 2), new Behavior_Bucket_Simple(ST.make(Items.milk_bucket, 1, 0))									, tData.copy(), FL.Milk.make(1000), FL.MilkGrC.make(1000)							, "container1000milk")
			, addItem(++tLastID, "Wooden Bucket", "Latex"									, TC.stack(TC.ARBOR, 2), TC.stack(TC.LIMUS		, 2), Behavior_Bucket_Simple.INSTANCE																, tData.copy(), FL.Latex.make(1000), UT.Fluids.make("molten.latex", 1000)			, "container1000latex")
			, addItem(++tLastID, "Wooden Bucket", "Creosote Oil"							, TC.stack(TC.ARBOR, 2), TC.stack(TC.IGNIS		, 2), new Behavior_Bucket_Simple(IL.RC_Creosote_Bucket.get(1))										, tData.copy(), FL.Oil_Creosote.make(1000)											, "container1000creosote")
			, addItem(++tLastID, "Wooden Bucket", "Sea Water (you cannot place this!)"		, TC.stack(TC.ARBOR, 2), TC.stack(TC.AQUA		, 2), new Behavior_Switch_Metadata(2000 + i*100)													, tData.copy(), FL.Ocean.make(1000), FL.OceanGrC.make(1000)							)
			, addItem(++tLastID, "Wooden Bucket", "Rubber Tree Sap"							, TC.stack(TC.ARBOR, 2), TC.stack(TC.LIMUS		, 2), Behavior_Bucket_Simple.INSTANCE																, tData.copy(), FL.Resin_Rubber.make(1000)											, "container1000rubbertreesap")
			, addItem(++tLastID, "Wooden Bucket", "Spruce Tree Resin"						, TC.stack(TC.ARBOR, 3), TC.stack(TC.LIMUS		, 1), Behavior_Bucket_Simple.INSTANCE																, tData.copy(), FL.Resin_Spruce.make(1000)											, "container1000spruceresin")
			, addItem(++tLastID, "Wooden Bucket", "Honey"									, TC.stack(TC.ARBOR, 3), TC.stack(TC.BESTIA		, 1), new Behavior_Bucket_Simple(ST.make(MD.ERE, "bucketHoney", 1, 0, IL.FR_Honey_Bucket.get(1)))	, tData.copy(), FL.Honey.make(1000), FL.HoneyGrC.make(1000), FL.HoneyBoP.make(1000)	, "container1000honey")
			...
			, addItem(++tLastID, "Wooden Bucket", "Lubricant"								, TC.stack(TC.ARBOR, 2), TC.stack(TC.LIMUS		, 2), Behavior_Bucket_Simple.INSTANCE																, tData.copy(), FL.LubRoCant.make(1000), FL.Lubricant.make(1000)					, "container1000lubricant")
		...
		RM.Canner.addRecipe1(T, 16, 16, IL.Tool_Lighter_Invar_Empty.get(1), MT.Butane .fluid(U/10, T), NF, IL.Tool_Lighter_Invar_Full.get(1));
		RM.Canner.addRecipe1(T, 16, 16, IL.Tool_Lighter_Invar_Empty.get(1), MT.Propane.fluid(U/10, T), NF, IL.Tool_Lighter_Invar_Full.get(1));
		...
		RM.Canner.addRecipe1(T, 16, 64, IL.Tool_Lighter_Platinum_Empty.get(1), MT.Butane .fluid(U, T), NF, IL.Tool_Lighter_Platinum_Full.get(1));
		RM.Canner.addRecipe1(T, 16, 64, IL.Tool_Lighter_Platinum_Empty.get(1), MT.Propane.fluid(U, T), NF, IL.Tool_Lighter_Platinum_Full.get(1));
		...
		RM.Canner.addRecipe1(T, 16, 16, IL.Tool_Lighter_Plastic_Empty.get(1), MT.Butane .fluid(U/10, T), NF, IL.Tool_Lighter_Plastic_Full.get(1));
		RM.Canner.addRecipe1(T, 16, 16, IL.Tool_Lighter_Plastic_Empty.get(1), MT.Propane.fluid(U/10, T), NF, IL.Tool_Lighter_Plastic_Full.get(1));
		...
		IL.Tool_Worldgen_Debugger.set(			addItem(tLastID =  9999, "Worldgen Debug Wand"		, "", Behavior_Worldgen_Debugger.INSTANCE, ItemsGT.DEBUG_ITEMS, ItemsGT.ILLEGAL_DROPS, TC.stack(TC.TERRA,10), TC.stack(TC.PRAECANTIO,10), TC.stack(TC.INSTRUMENTUM,10)));
...
//		Moved to the Battery Section ofthe MTE Loader.
...
//		CR.shaped(IL.Tool_Cropnalyzer			.get(1), CR.DEF_REV, "EXR", "CPU", "BXB", 'B', IL.Battery_Alkaline_MV, 'X', OP.plate.dat(MT.Al), 'U', OD_USB_STICKS[1], 'C', OD_USB_CABLES[1], 'E', IL.EMITTERS[2], 'R', IL.SENSORS[2], 'P', OD_CIRCUITS[6]);
		...
		IL.Compass_North.set(					addItem(tLastID = 11000, "Compass (NORTH)"		, "Sneak Rightclick to switch Mode", "itemCompass", new Behavior_Switch_Metadata(11001), OM.data(ST.make(Items.compass, 1, 0)), TC.stack(TC.METALLUM, 2), TC.stack(TC.MAGNETO													, 2))); CR.shapeless(ST.make(Items.compass, 1, 0), CR.DEF, new Object[] {ST.make(this, 1, tLastID)});
		IL.Compass_Face.set(					addItem(tLastID = 11001, "Compass (FACE)"		, "Sneak Rightclick to switch Mode", "itemCompass", new Behavior_Switch_Metadata(11002), OM.data(ST.make(Items.compass, 1, 0)), TD.Creative.HIDDEN, TC.stack(TC.METALLUM, 2), TC.stack(TC.MAGNETO, 1), TC.stack(TC.COGNITO		, 1))); CR.shapeless(ST.make(Items.compass, 1, 0), CR.DEF, new Object[] {ST.make(this, 1, tLastID)});
		IL.Compass_Spawn.set(					addItem(tLastID = 11002, "Compass (SPAWN)"		, "Sneak Rightclick to switch Mode", "itemCompass", new Behavior_Switch_Metadata(11003), OM.data(ST.make(Items.compass, 1, 0)), TD.Creative.HIDDEN, TC.stack(TC.METALLUM, 2), TC.stack(TC.MAGNETO, 1), TC.stack(TC.HUMANUS		, 1))); CR.shapeless(ST.make(Items.compass, 1, 0), CR.DEF, new Object[] {ST.make(this, 1, tLastID)});
		IL.Compass_Center.set(					addItem(tLastID = 11003, "Compass (CENTER)"		, "Sneak Rightclick to switch Mode", "itemCompass", new Behavior_Switch_Metadata(11004), OM.data(ST.make(Items.compass, 1, 0)), TD.Creative.HIDDEN, TC.stack(TC.METALLUM, 2), TC.stack(TC.MAGNETO, 1), TC.stack(TC.TERRA		, 1))); CR.shapeless(ST.make(Items.compass, 1, 0), CR.DEF, new Object[] {ST.make(this, 1, tLastID)});
		IL.Compass_Death.set(					addItem(tLastID = 11004, "Compass (DEATH)"		, "Sneak Rightclick to switch Mode", "itemCompass", new Behavior_Switch_Metadata(11000), OM.data(ST.make(Items.compass, 1, 0)), TD.Creative.HIDDEN, TC.stack(TC.METALLUM, 2), TC.stack(TC.MAGNETO, 1), TC.stack(TC.MORTUUS		, 1))); CR.shapeless(ST.make(Items.compass, 1, 0), CR.DEF, new Object[] {ST.make(this, 1, tLastID)});
		...
		IL.Key_Iron.set(						addItem(tLastID = 30000, "Iron Key"		, "", new OreDictItemData(ANY.Iron			, U4), Behavior_Key.INSTANCE, TC.stack(TC.MACHINA, 1))); CR.shaped(IL.Key_Iron		.get(3), CR.DEF_NCC, "fPx", 'P', OP.plate.dat(ANY.Iron));
		IL.Key_Gold.set(						addItem(tLastID = 30001, "Gold Key"		, "", new OreDictItemData(MT.Au				, U4), Behavior_Key.INSTANCE, TC.stack(TC.MACHINA, 1))); CR.shaped(IL.Key_Gold		.get(3), CR.DEF_NCC, "fPx", 'P', OP.plate.dat(MT.Au));
		IL.Key_Copper.set(						addItem(tLastID = 30002, "Copper Key"	, "", new OreDictItemData(ANY.Cu			, U4), Behavior_Key.INSTANCE, TC.stack(TC.MACHINA, 1))); CR.shaped(IL.Key_Copper	.get(3), CR.DEF_NCC, "fPx", 'P', OP.plate.dat(ANY.Cu));
		IL.Key_Tin.set(							addItem(tLastID = 30003, "Tin Key"		, "", new OreDictItemData(MT.Sn				, U4), Behavior_Key.INSTANCE, TC.stack(TC.MACHINA, 1))); CR.shaped(IL.Key_Tin		.get(3), CR.DEF_NCC, "fPx", 'P', OP.plate.dat(MT.Sn));
		IL.Key_Bronze.set(						addItem(tLastID = 30004, "Bronze Key"	, "", new OreDictItemData(MT.Bronze			, U4), Behavior_Key.INSTANCE, TC.stack(TC.MACHINA, 1))); CR.shaped(IL.Key_Bronze	.get(3), CR.DEF_NCC, "fPx", 'P', OP.plate.dat(MT.Bronze));
		IL.Key_Brass.set(						addItem(tLastID = 30005, "Brass Key"	, "", new OreDictItemData(MT.Brass			, U4), Behavior_Key.INSTANCE, TC.stack(TC.MACHINA, 1))); CR.shaped(IL.Key_Brass		.get(3), CR.DEF_NCC, "fPx", 'P', OP.plate.dat(MT.Brass));
		IL.Key_Silver.set(						addItem(tLastID = 30006, "Silver Key"	, "", new OreDictItemData(MT.Ag				, U4), Behavior_Key.INSTANCE, TC.stack(TC.MACHINA, 1))); CR.shaped(IL.Key_Silver	.get(3), CR.DEF_NCC, "fPx", 'P', OP.plate.dat(MT.Ag));
		IL.Key_Platinum.set(					addItem(tLastID = 30007, "Platinum Key"	, "", new OreDictItemData(MT.Pt				, U4), Behavior_Key.INSTANCE, TC.stack(TC.MACHINA, 1))); CR.shaped(IL.Key_Platinum	.get(3), CR.DEF_NCC, "fPx", 'P', OP.plate.dat(MT.Pt));
		IL.Key_Lead.set(						addItem(tLastID = 30008, "Lead Key"		, "", new OreDictItemData(MT.Pb				, U4), Behavior_Key.INSTANCE, TC.stack(TC.MACHINA, 1))); CR.shaped(IL.Key_Lead		.get(3), CR.DEF_NCC, "fPx", 'P', OP.plate.dat(MT.Pb));
		IL.Key_Plastic.set(						addItem(tLastID = 30009, "Plastic Key"	, "", new OreDictItemData(MT.Plastic		, U4), Behavior_Key.INSTANCE, TC.stack(TC.MACHINA, 1))); CR.shaped(IL.Key_Plastic	.get(3), CR.DEF_NCC, "fPx", 'P', OP.plate.dat(MT.Plastic));
