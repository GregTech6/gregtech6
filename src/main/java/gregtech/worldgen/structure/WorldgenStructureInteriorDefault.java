...
public class WorldgenStructureInteriorDefault extends WorldgenStructure {
	public static boolean generate(World aWorld, Random aRandom, int aChunkX, int aChunkZ, StructureData aData) {
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+ 3, aData.mOffsetY+1, aChunkZ+ 1, SIDE_UNKNOWN, (short)   11, UT.NBT.make(null, NBT_COLOR, DYES_INT[aData.mColor], NBT_PAINTED, T, NBT_FACING, SIDE_Z_POS, "gt.dungeonloot", ChestGenHooks.MINESHAFT_CORRIDOR		), T, T);
		
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+ 2, aData.mOffsetY+1, aChunkZ+ 1, SIDE_UNKNOWN, (short)   11, UT.NBT.make(null, NBT_COLOR, DYES_INT[aData.mColor], NBT_PAINTED, T, NBT_FACING, SIDE_Z_POS, "gt.dungeonloot", ChestGenHooks.STRONGHOLD_CROSSING		), T, T);
		
		setBlock									(aWorld, aChunkX+ 1, aData.mOffsetY+1, aChunkZ+ 1, Blocks.crafting_table, 0, 2);
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+ 1, aData.mOffsetY+2, aChunkZ+ 1, SIDE_UNKNOWN, (short)32735, UT.NBT.make(), T, T);
		
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+ 1, aData.mOffsetY+1, aChunkZ+ 2, SIDE_UNKNOWN, (short)   11, UT.NBT.make(null, NBT_COLOR, DYES_INT[aData.mColor], NBT_PAINTED, T, NBT_FACING, SIDE_X_POS, "gt.dungeonloot", ChestGenHooks.DUNGEON_CHEST			), T, T);
		...
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+ 1, aData.mOffsetY+1, aChunkZ+ 3, SIDE_UNKNOWN, (short) 4011, UT.NBT.make(null, NBT_COLOR, DYES_INT[aData.mColor], NBT_PAINTED, T, NBT_FACING, SIDE_X_POS, NBT_INV_LIST, tInventory), T, T);
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+ 1, aData.mOffsetY+2, aChunkZ+ 3, SIDE_UNKNOWN, (short) 2010, UT.NBT.make(null, NBT_COLOR, DYES_INT[aData.mColor], NBT_PAINTED, T, NBT_FACING, SIDE_X_POS, "gt.dungeonloot", ChestGenHooks.PYRAMID_JUNGLE_CHEST	), T, T);
		setCoins									(aWorld, aChunkX+ 1, aData.mOffsetY+3, aChunkZ+ 3, aData, aRandom);
		...
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+ 4, aData.mOffsetY+1, aChunkZ+ 1, SIDE_UNKNOWN, (short) 6011, UT.NBT.make(null, NBT_COLOR, DYES_INT[aData.mColor], NBT_PAINTED, T, NBT_FACING, SIDE_Z_POS, NBT_INV_LIST, tInventory), T, T);
		tInventory = new NBTTagList();
		tInventory.appendTag(UT.NBT.makeShort(ST.save(ST.make(aData.mSecondary	,  1001+aRandom.nextInt( 9000), 1)), "s", (short)1));
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+ 4, aData.mOffsetY+2, aChunkZ+ 1, SIDE_UNKNOWN, (short) 6011, UT.NBT.make(null, NBT_COLOR, DYES_INT[aData.mColor], NBT_PAINTED, T, NBT_FACING, SIDE_Z_POS, NBT_INV_LIST, tInventory), T, T);
		tInventory = new NBTTagList();
		...
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+ 4, aData.mOffsetY+3, aChunkZ+ 1, SIDE_UNKNOWN, (short) 7111, UT.NBT.make(null, NBT_COLOR, DYES_INT[aData.mColor], NBT_PAINTED, T, NBT_FACING, SIDE_Z_POS, NBT_INV_LIST, tInventory, "gt.dungeonloot.front", ChestGenHooks.VILLAGE_BLACKSMITH), T, T);
		...
		setSmoothBlock								(aWorld, aChunkX+14, aData.mOffsetY+1, aChunkZ+ 2, aData, aRandom);
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+14, aData.mOffsetY+1, aChunkZ+ 3, SIDE_UNKNOWN, (short)(1102+tCrucibleType), UT.NBT.make(null, NBT_FACING, SIDE_X_NEG), T, T);
		setSmoothBlock								(aWorld, aChunkX+14, aData.mOffsetY+1, aChunkZ+ 4, aData, aRandom);
		setBlock									(aWorld, aChunkX+11, aData.mOffsetY+1, aChunkZ+ 1, Blocks.anvil, 3 | (aRandom.nextInt(3) << 2), 0);
		
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+14, aData.mOffsetY+2, aChunkZ+ 2, SIDE_UNKNOWN, (short)(1070+tCrucibleType), UT.NBT.make(null, "gt.mold", tMoldShapes.isEmpty()?0:tMoldShapes.get(aRandom.nextInt(tMoldShapes.size()))), T, T);
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+14, aData.mOffsetY+2, aChunkZ+ 3, SIDE_UNKNOWN, (short)(1020+tCrucibleType), UT.NBT.make(null, NBT_FACING, SIDE_X_NEG), T, T);
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+14, aData.mOffsetY+2, aChunkZ+ 4, SIDE_UNKNOWN, (short)(1070+tCrucibleType), UT.NBT.make(null, "gt.mold", tMoldShapes.isEmpty()?0:tMoldShapes.get(aRandom.nextInt(tMoldShapes.size()))), T, T);
		
		//-----
		
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+13, aData.mOffsetY+1, aChunkZ+14, SIDE_UNKNOWN, (short)32705, null, T, T);
		setSmoothBlock								(aWorld, aChunkX+14, aData.mOffsetY+1, aChunkZ+14, aData, aRandom);
		setBlock									(aWorld, aChunkX+14, aData.mOffsetY+1, aChunkZ+13, Blocks.cauldron, aRandom.nextInt(4), 0);
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+14, aData.mOffsetY+1, aChunkZ+11, SIDE_UNKNOWN, (short)32707, null, T, T);
		
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+13, aData.mOffsetY+2, aChunkZ+14, SIDE_UNKNOWN, (short)32730, UT.NBT.make(null, NBT_FACING, SIDE_X_POS), T, T);
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+14, aData.mOffsetY+2, aChunkZ+14, SIDE_UNKNOWN, (short)32716, new FluidTankGT(FL.Water.make(64000)).writeToNBT(UT.NBT.make(), NBT_TANK), T, T);
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+14, aData.mOffsetY+2, aChunkZ+13, SIDE_UNKNOWN, (short)32730, UT.NBT.make(null, NBT_FACING, SIDE_Z_POS), T, T);
		
		aData.mRegistry.mBlock.placeBlock			(aWorld, aChunkX+14, aData.mOffsetY+3, aChunkZ+14, SIDE_UNKNOWN, (short)32725, UT.NBT.make(null, NBT_FACING, SIDE_BOTTOM), T, T);
		...
		for (int i = 0; i < 2; i++) for (int j = 0; j < 2; j++) if (aRandom.nextInt(3) > 0) for (int k = 0; k < 3; k++) {
			aData.mRegistry.mBlock.placeBlock(aWorld, aChunkX+1+i, aData.mOffsetY+1+k, aChunkZ+12+j, SIDE_UNKNOWN, tID, new FluidTankGT(UT.Code.select(NF, tDrinks)).writeToNBT(UT.NBT.make(), NBT_TANK), T, T);
